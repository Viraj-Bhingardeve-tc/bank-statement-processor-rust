...
//! extractor.
//!
//! Phase 4D adds `license_service`, constructed once here from the
//! Postgres-backed repositories so every handler shares one instance rather
//! than building its own per request. A Razorpay HTTP client lands here in
//! a later phase (`PHASE4_DESIGN.md` §1.2's "External" layer) — each
//! handler extracts only the fields it actually needs via
//! `State<AppState>`, so adding a field here never forces every existing
//! handler to change.
