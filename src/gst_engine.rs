...
static RATE_PATTERNS: Lazy<Vec<(f64, Regex)>> = Lazy::new(|| {
    vec![
        (
            28.0,
            Regex::new(r"(?i)\b28\s*%?\s*(gst|tax)\b|gst\s*@?\s*28\b").unwrap(),
        ),
        (
            18.0,
            Regex::new(r"(?i)\b18\s*%?\s*(gst|tax)\b|gst\s*@?\s*18\b").unwrap(),
        ),
        (
            12.0,
            Regex::new(r"(?i)\b12\s*%?\s*(gst|tax)\b|gst\s*@?\s*12\b").unwrap(),
        ),
        (
            5.0,
            Regex::new(r"(?i)\b5\s*%?\s*(gst|tax)\b|gst\s*@?\s*5\b").unwrap(),
        ),
    ]
});
...
static RE_GST_MENTION: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bGST\b|\bIGST\b|\bCGST\b|\bSGST\b").unwrap());
...
static VENDOR_GST_MAP: Lazy<Vec<VendorGst>> = Lazy::new(|| {
    vec![
        VendorGst {
            kw: &[
                "AIRTEL",
        ...
        VendorGst {
            kw: &["RENT", "OFFICE RENT", "SHOP RENT", "RENTAL", "LANDLORD"],
            rate: 18.0,
            gst_type: "CGST+SGST",
            expense: "Rent",
        },
        ...
        VendorGst {
            kw: &["UBER", "OLA", "RAPIDO", "MERU", "TAXISURE"],
            rate: 5.0,
            gst_type: "CGST+SGST",
            expense: "Travelling Expense",
        },
        VendorGst {
            kw: &[
                "FACEBOOK ADS",
...
fn detect_component(text: &str) -> Option<&'static str> {
    if RE_IGST.is_match(text) {
        return Some("IGST");
    }
    if RE_CGST.is_match(text) || RE_SGST.is_match(text) {
        return Some("CGST+SGST");
    }
    None
}
...
fn vendor_match(text_upper: &str) -> Option<&'static VendorGst> {
    VENDOR_GST_MAP
        .iter()
        .find(|v| v.kw.iter().any(|k| text_upper.contains(k)))
}
...
pub fn analyse(
    ...
    let gst_type = component
        .map(|s| s.to_string())
        .or_else(|| vendor_hit.map(|v| v.gst_type.to_string()))
        .or_else(|| {
            if gst_rate.is_some() {
                Some("CGST+SGST".to_string())
            } else {
                None
            }
        });
    ...
    if rate_from_txt.is_some() {
        confidence += 0.20;
    }
    if vendor_hit.is_some() {
        confidence += 0.15;
    }
    if gst_mentioned {
        confidence += 0.10;
