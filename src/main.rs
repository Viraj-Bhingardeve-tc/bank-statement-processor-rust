...
use bank_statement_processor::{
    analytics, auth, classifier, db, export, license, migration, narration_cleaner, parser,
    reconciliation, settings, tally_group_engine, ui,
};

use std::sync::{Arc, Mutex};

#[cfg(feature = "slint-ui")]
use slint::{Model as _, SharedString};
...
#[cfg(feature = "slint-ui")]
fn read_workbook_grid(
    mut wb: calamine::Sheets<std::io::BufReader<std::fs::File>>,
) -> Vec<Vec<String>> {
    use calamine::Reader;
...
fn apply_txn_filters<'a>(
    ...
    txns.iter()
        ...
        .filter(|t| {
            ...
            let td = match parse_date_ymd(&t.date) {
                Some(d) => d,
                None => return true,
            };
            if !from.is_empty() {
                if let Some(fd) = parse_date_ymd(from) {
                    if td < fd {
                        return false;
                    }
                }
            ...
            if !to.is_empty() {
                if let Some(td2) = parse_date_ymd(to) {
                    if td > td2 {
                        return false;
                    }
                }
...
fn build_txn_rows(txns: &[&parser::Transaction]) -> Vec<TxnRow> {
    ...
    txns.iter()
        ...
        .map(|(idx, t)| {
            ...
            let row_color: i32 = if effective_needs_review
                && !matches!(t.status, parser::TransactionStatus::NeedsReview)
            {
...
fn compute_filter_counts(txns: &[parser::Transaction]) -> [usize; 7] {
    ...
    let suspense = real
        .iter()
        .filter(|t| matches!(t.status, parser::TransactionStatus::Suspense))
        .count();
    let high = real
        .iter()
        .filter(|t| {
            matches!(t.status, parser::TransactionStatus::Classified) && t.confidence >= 0.7
        })
        .count();
    ...
    let review = real
        .iter()
        .filter(|t| analytics::effective_needs_review(t))
        .count();
...
fn rebuild_rows(h: &AppWindow, st: &ui::AppState) {
    ...
    log::info!(
        "[Filter] showing {} / {} txns  (statuses={:?} from='{}' to='{}' bank='{}')",
        filtered.len(),
        st.transactions
            .iter()
            .filter(|t| !t.is_opening_balance)
...
fn fy_range(current: bool) -> (String, String) {
    ...
    if current {
        (
            format!("01/04/{}", fy_start_year),
            format!("31/03/{}", fy_end_year),
        )
    } else {
        (
            format!("01/04/{}", fy_start_year - 1),
            format!("31/03/{}", fy_start_year),
        )
    }
}
...
fn push_dashboard(h: &AppWindow, txns: &[parser::Transaction], opening_bal: Option<f64>) {
    use analytics::{compute, fmt_amt, unique_banks, unique_heads, unique_vendors};
    ...
    h.set_dash_top_exp_amt(SharedString::from(
        if s.top_expense_amt > 0.0 {
            fmt_amt(Some(s.top_expense_amt))
        } else {
            String::new()
        }
        .as_str(),
    ...
    h.set_dash_needs_review(SharedString::from(
        s.needs_review_count.to_string().as_str(),
    ));
        ...
        .map(|v| {
            ...
            DashVendorBar {
                ...
                debit_str: SharedString::from(
                    if v.debit > 0.0 {
                        analytics::fmt_short_pub(v.debit)
                    } else {
                        String::new()
                    }
                    .as_str(),
                ...
                credit_str: SharedString::from(
                    if v.credit > 0.0 {
                        analytics::fmt_short_pub(v.credit)
                    } else {
                        String::new()
                    }
                    .as_str(),
...
fn push_summary_extras(h: &AppWindow, txns: &[parser::Transaction]) {
    ...
    let real: Vec<&parser::Transaction> = txns.iter().filter(|t| !t.is_opening_balance).collect();
    if real.is_empty() {
        h.set_dash_parties(slint::ModelRc::new(
        ...
        h.set_dash_pay_ledgers(slint::ModelRc::new(
            slint::VecModel::<SummaryListRow>::from(vec![]),
        ));
        return;
    }
    ...
    let ai_cnt = real
        .iter()
        .filter(|t| t.classification_source == "ai")
        .count();
    let rule_cnt = real
        .iter()
        .filter(|t| t.classification_source == "rule")
        .count();
    let kw_cnt = real
        .iter()
        .filter(|t| t.classification_source == "keyword")
        .count();
    ...
    // ── GST paid / received ───────────────────────────────────────────────────
    let gst_txns: Vec<&&parser::Transaction> = real
        .iter()
        .filter(|t| {
            t.tags.iter().any(|g| {
                let u = g.to_uppercase();
    ...
    let mut parties: Vec<(String, usize, f64)> = party_map
        .into_iter()
        .map(|(k, (cnt, amt))| (k, cnt, amt))
        .collect();
    parties.sort_by(|a, b| b.1.cmp(&a.1));
    parties.truncate(6);
    let party_rows: Vec<SummaryListRow> = parties
        .iter()
        .map(|(name, cnt, amt)| {
    ...
    let account_rows: Vec<AccountSummaryRow> = acc_order
        ...
        .map(|(bank, acct)| {
            let acc_txns: Vec<&parser::Transaction> = txns
                ...
                .collect();
            let opening = acc_txns
                .iter()
                .find(|t| t.is_opening_balance)
                .and_then(|t| t.balance);
            let mut non_ob: Vec<&&parser::Transaction> =
    ...
    h.set_dash_rec_ledgers(slint::ModelRc::new(slint::VecModel::from(
        make_ledger_rows(rec_map),
    )));
    h.set_dash_pay_ledgers(slint::ModelRc::new(slint::VecModel::from(
        make_ledger_rows(pay_map),
    )));
}
...
fn finish_load_file(
    ...
    if h.get_dedup_enabled() {
        ...
        if let Some(cid) = client_id {
            ...
            result
                .transactions
                .retain(|t| t.is_opening_balance || !known.contains(&t.hash()));
            let after = result
                .transactions
                .iter()
                .filter(|t| !t.is_opening_balance)
            ...
            let new_hashes: Vec<String> = result
                .transactions
                .iter()
                .filter(|t| !t.is_opening_balance)
                .map(|t| t.hash())
...
fn apply_parse_result(
    ...
    let calc_closing = result
        .opening_balance
        .map(|ob| (ob + total_cr - total_dr).round() / 1.0);
    ...
    let cleaned_narrations: Vec<narration_cleaner::NarrationMeta> = if cfg.narr_enabled {
        ...
        narration_strs
            .iter()
            .map(|n| narration_cleaner::NarrationMeta {
                original: n.clone(),
                cleaned: n.clone(),
    ...
    let validation: Vec<(bool, String)> = real
        .iter()
        .map(|t| analytics::validate_transaction(t))
        .collect();
    ...
    let import_id_persisted: Option<i64> = {
        ...
        if let Some(cid) = client_id {
            ...
            if let Some(conn) = db.as_ref() {
                ...
                if let Some(iid) = imp_id {
                    match db::upsert_transactions(conn, cid, Some(iid), &result.transactions) {
                        ...
                        Err(e) => {
                            ...
                            h.set_toast_msg(SharedString::from(
                                format!("Save failed — transactions were NOT saved: {}", e)
                                    .as_str(),
                            ));
                            h.set_toast_kind(2);
                        }
                ...
                let heads_with_groups: Vec<(String, String)> = real
                    .iter()
...
fn record_batch_success(
    ...
    let non_ob: Vec<&parser::Transaction> =
        r_txns.iter().filter(|t| !t.is_opening_balance).collect();
    let first_d = non_ob
        .first()
        .map(|t| t.date.as_str())
        .unwrap_or("")
    ...
    let last_d = non_ob
        ...
        .unwrap_or("")
        .to_string();
    let period = if first_d.is_empty() {
        "—".to_string()
    } else if first_d == last_d {
        first_d.clone()
    } else {
        format!("{} - {}", first_d, last_d)
    };
    ...
    let new_txns: Vec<parser::Transaction> = r
        .transactions
        .into_iter()
    ...
    bp.loaded += 1;
    if bp.first_bank.is_empty() {
        bp.first_bank = r_bank.clone();
    }
    if bp.first_ob.is_none() {
        bp.first_ob = r_ob;
    }
...
fn finish_batch(
    ...
    if let Some(cid) = client_id {
        let new_hashes: Vec<String> = all_txns
            .iter()
            .filter(|t| !t.is_opening_balance)
            .map(|t| t.hash())
            .collect();
    ...
    let real: Vec<&parser::Transaction> =
        all_txns.iter().filter(|t| !t.is_opening_balance).collect();
    let total_dr: f64 = real.iter().filter_map(|t| t.debit).sum();
    let total_cr: f64 = real.iter().filter_map(|t| t.credit).sum();
    let row_models = build_txn_rows(&real);
    let bank_set: std::collections::BTreeSet<String> =
        real.iter().map(|t| t.bank_name.clone()).collect();
    let bank_names: Vec<SharedString> = std::iter::once(SharedString::from("All Banks"))
        .chain(bank_set.iter().map(|b| SharedString::from(b.as_str())))
        .collect();
    ...
    let batch_event = format!(
        "[{}] Import — {} file(s), {} transactions loaded",
        audit_now(),
        loaded,
        real.len()
    );
    ...
    let (summary, toast_kind) =
        batch_summary_message(loaded, skipped, errors, aborted, unprocessed);
    ...
    match try_parse_batch_file(&path, &file_name) {
        ...
        BatchFileOutcome::NeedsPassword => {
            ...
            h.set_pdf_pwd_prompt(SharedString::from(
                format!(
                    "'{}' is password-protected. Enter the PDF password:",
                    file_name
                )
                .as_str(),
            ));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db_conn: Arc<Mutex<Option<rusqlite::Connection>>> =
        Arc::new(Mutex::new(match db::open(&db_path) {
            Ok(c) => {
    ...
    #[cfg(feature = "slint-ui")]
    {
        ...
        if let Some(err) = &db_open_error {
            app.set_toast_msg(SharedString::from(
                format!(
                    "Database unavailable — imports/saves will not work until this is fixed: {err}"
                )
                .as_str(),
            ));
        ...
        let app_state: Arc<Mutex<ui::AppState>> = Arc::new(Mutex::new(ui::AppState {
            dedup_enabled: true,
            ..Default::default()
        }));
        ...
        // ── Load clients from DB into dropdown on startup ─────────────────────
        {
            ...
            if let Some(conn) = db.as_ref() {
                ...
                app.set_settings_recon_days(SharedString::from(
                    cfg.recon_days.to_string().as_str(),
                ));
                ...
                if ok {
                    ...
                    std::thread::spawn(move || {
                        ...
                        let _ = slint::invoke_from_event_loop(move || {
                            let Some(h) = handle_for_result.upgrade() else {
                                return;
                            };
            ...
            app.on_do_load_file(move || {
                ...
                if ext == "pdf" {
                    ...
                    let stage1 = match parser::text_extractor::extract_pages(&path) {
                        Ok(rows) => {
                            if rows.is_empty() {
                                ...
                                parser::pdf_parser::parse_pdf_rows(rows, &file_name)
                            }
                        }
                        Err(e) => {
                            let emsg = e.to_string();
                            if emsg.contains("password-protected")
                                || emsg.to_lowercase().contains("encrypt")
                            {
                                {
                                    let mut st = state_ref.lock().unwrap();
                                    st.pending_pdf_path = Some(path.clone());
                    ...
                    std::thread::spawn(move || {
                        ...
                        let _ = slint::invoke_from_event_loop(move || {
                            ...
                            match outcome {
                                Ok(r) => {
                                    finish_load_file(&h2, &state_ref2, &db_ref2, r, &file_name2)
                                }
                ...
                if parser::ocr_extractor::IMAGE_EXTS.contains(&ext.as_str()) {
                    ...
                    std::thread::spawn(move || {
                        let progress_handle = handle2.clone();
                        let outcome =
                            run_image_ocr_pipeline(&path2, &file_name2, move |pct, msg| {
                                let h = progress_handle.clone();
                                let msg = msg.to_string();
                                let _ = slint::invoke_from_event_loop(move || {
                                    if let Some(h) = h.upgrade() {
                ...
                }
                {
                    ...
                    if let Some(conn) = db.as_ref() {
                        if let Ok(clients) = db::get_clients(conn) {
                            if clients
                                .iter()
                                .any(|c| c.name.eq_ignore_ascii_case(name.trim()))
                            {
                                drop(db);
                ...
                let new_id = {
                    ...
                    if let Some(conn) = db.as_ref() {
                        match db::add_client(conn, name.trim(), ledger.trim()) {
                            Ok(id) => {
                                log::info!(
                                    "[NewClient] created id={} name='{}' ledger='{}'",
                                    id,
                                    name,
                                    ledger
                                );
                            ...
                            Err(e) => {
                                log::error!("[NewClient] DB error: {}", e);
                                None
                            }
                        }
                    } else {
                    ...
                    if let Some(conn) = db.as_ref() {
                        if let Ok(clients) = db::get_clients(conn) {
                            let names: Vec<SharedString> =
                                std::iter::once(SharedString::from("-- Select Client --"))
                                    .chain(
                                        clients.iter().map(|c| SharedString::from(c.name.as_str())),
                                    )
                                    .collect();
        ...
        // ── Auto-Classify All ─────────────────────────────────────────────────
        {
            ...
            app.on_do_auto_classify(move || {
                ...
                log::info!(
                    "[AutoClassify] classified {} transactions (rules={})",
                    changed,
                    rules.len()
                );
        ...
        // ── Delete Rule ───────────────────────────────────────────────────────
        {
            ...
            app.on_do_delete_rule(move |idx| {
                ...
                let rules = match db::get_rules(conn, client_id) {
                    Ok(r) => r,
                    Err(e) => {
                        log::error!("[DeleteRule] DB error: {}", e);
                        return;
                    }
                };
                let Some(rule) = rules.get(idx as usize) else {
                    log::warn!(
                        "[DeleteRule] idx {} out of range (len={})",
                        idx,
                        rules.len()
                    );
                    return;
                ...
                if let Some(conn) = db.as_ref() {
                    match db::get_imports(conn, client_id) {
                        Ok(imports) => {
                            ...
                            let recs: Vec<SharedString> = imports
                                .iter()
                                .map(|imp| {
                            ...
                            log::info!(
                                "[ImportHistory] loaded {} records for client_id={}",
                                imports.len(),
                                client_id
                            );
                        }
        ...
        // ── Delete Import ────────────────────────────────────────────────────
        {
            ...
            app.on_do_delete_import(move |idx| {
                ...
                let imports = match db::get_imports(conn, client_id) {
                    Ok(i) => i,
                    Err(e) => {
                        log::error!("[DeleteImport] DB error: {}", e);
                        return;
                    }
                };
                let Some(import) = imports.get(idx as usize) else {
                    log::warn!(
                        "[DeleteImport] idx {} out of range (len={})",
                        idx,
                        imports.len()
                    );
                    return;
                ...
                if let Err(e) = db::delete_import(conn, import.id) {
                    log::error!(
                        "[DeleteImport] failed to delete import {}: {}",
                        import.id,
                        e
                    );
                    return;
            ...
            app.on_do_import_ledgers(move || {
                ...
                let rows: Vec<Vec<String>> = if is_csv {
                    ...
                    let sheet_name = match wb.sheet_names().first() {
                        Some(n) => n.clone(),
                        None => {
                            h.set_toast_msg(SharedString::from("File has no sheets"));
                            h.set_toast_kind(2);
                            return;
                        }
                    };
                ...
                let group_col = header
                    .iter()
                    .position(|h| group_keys.iter().any(|k| h == k));

                let name_col = match name_col {
                    Some(c) => c,
                    None => {
                        h.set_toast_msg(SharedString::from(
                            "No 'Name' or 'Ledger Name' column found",
                        ));
                        h.set_toast_kind(2);
                        return;
                    }
                ...
                if let Some(conn) = db.as_ref() {
                    match db::import_ledgers(conn, cid, &entries) {
                        Ok(added) => {
                            ...
                            let msg = format!(
                                "Imported {} new ledgers ({} already existed)",
                                added,
                                total - added
                            );
                        ...
                        Err(e) => {
                            ...
                            h.set_toast_msg(SharedString::from(
                                "Failed to save ledgers to database",
                            ));
                            h.set_toast_kind(2);
                        }
                    }
        ...
        }
        {
            ...
            app.on_do_bank_filter(move |bank| {
                ...
                let mut st = state_ref.lock().unwrap();
                st.bank_filter = if bank == "All Banks" {
                    String::new()
                } else {
                    bank.to_string()
                };
        ...
        // ── Export Tally XML ──────────────────────────────────────────────────
        {
            ...
            app.on_do_export_tally(move || {
                ...
                let client_name = if company.is_empty() {
                    st.client_name.clone()
                } else {
                    company
                };
        ...
        // ── Accounting Export Wizard ──────────────────────────────────────────
        {
            ...
            app.on_do_export_accounting(move || {
                ...
                const STATE_OPTS: [&str; 9] =
                    ["MH", "GJ", "DL", "KA", "TN", "TG", "RJ", "UP", "WB"];
                let fy = FY_OPTS
                    .get(h.get_wiz_fy_idx() as usize)
                    .copied()
                    .unwrap_or("2024-25")
                    .to_string();
                let state_code = STATE_OPTS
                    .get(h.get_wiz_state_idx() as usize)
                    .copied()
                    .unwrap_or("MH")
                    .to_string();
        ...
        // ── Export wizard preview ─────────────────────────────────────────────
        {
            ...
            app.on_do_export_preview(move || {
                ...
                if p.contra > 0 {
                    parts.push(format!("{} Contra", p.contra));
                }
                if p.gst > 0 {
                    parts.push(format!("{} GST (\u{20b9}{:.2})", p.gst, p.gst_amount));
                }
                if p.skipped > 0 {
...

        {
            ...
            app.on_do_reimport_excel(move || {
                ...
                let mut wb = match open_workbook_auto(&path) {
                    ...
                    Err(e) => {
                        h.set_toast_msg(SharedString::from(
                            format!("Cannot open file: {}", e).as_str(),
                        ));
                        h.set_toast_kind(2);
                        return;
                    }
                ...
                let range = match wb.worksheet_range("Transactions") {
                    ...
                    Err(_) => {
                        h.set_toast_msg(SharedString::from(
                            "'Transactions' sheet not found — export from this app first",
                        ));
                        h.set_toast_kind(2);
                        return;
                    }
                ...
                if i_vend.is_none() || i_head.is_none() {
                    h.set_toast_msg(SharedString::from(
                        "Vendor and Account Head columns required — export from this app first",
                    ));
                    h.set_toast_kind(2);
                    return;
                }
                ...
                for row in rows.iter().skip(1) {
                    ...
                    let narr = get(i_narr);
                    let date = get(i_date);
                    if narr.is_empty() && date.is_empty() {
                        continue;
                    }
                    ...
                    let status = match stat_s.as_str() {
                        ...
                        _ => {
                            if vendor.is_empty() && head.is_empty() {
                    ...
                    let confidence: f64 = conf_s.parse().unwrap_or(
                        if matches!(status, parser::TransactionStatus::Classified) {
                            1.0
                        } else {
                            0.0
                        },
                ...
                let msg = format!(
                    "Re-imported {} transactions ({} pre-classified)",
                    total_cnt, classified_cnt
                );
                h.set_toast_msg(SharedString::from(msg.as_str()));
                h.set_toast_kind(1);
        ...
        // ── Reset Dedupe ──────────────────────────────────────────────────────
        {
            ...
            app.on_do_reset_dedupe(move || {
                ...
                let event_str = format!(
                    "[{}] Reset Dedupe — {} duplicate(s) redetected",
                    audit_now(),
                    dup_count
                );
            ...
            app.on_do_migration_run(move || {
                ...
                std::thread::spawn(move || {
                    ...
                    if let Err(e) = &reopened {
                        log::error!(
                            "[Migration] failed to reopen the database after migrating: {}",
                            e
                        );
                    }
                    {
                    ...
                    let _ = slint::invoke_from_event_loop(move || {
                        ...
                        match report {
                            ...
                            Err(e) => {
                                ...
                                h2.set_toast_msg(SharedString::from(
                                    format!("Migration could not run: {}", e).as_str(),
                                ));
                                h2.set_toast_kind(2);
                            }
                        }
                            ...
                            if let Ok(clients) = db::get_clients(conn) {
                                let names: Vec<SharedString> =
                                    std::iter::once(SharedString::from("-- Select Client --"))
                                        ...
                                        .collect();
                                h2.set_client_names(slint::ModelRc::new(slint::VecModel::from(
                                    names,
                                )));
                            }
                        }
        ...
        // ── Legacy Data Migration: save the full report to disk ────────────────
        {
            ...
            app.on_do_migration_save_report(move || {
                ...
                match std::fs::write(&path, md) {
                    Ok(_) => {
                        h.set_toast_msg(SharedString::from(
                            format!("Report saved to {}", path.display()).as_str(),
                        ));
                        h.set_toast_kind(1);
                    }
                    Err(e) => {
        ...
        // ── Load All from DB ──────────────────────────────────────────────────
        {
            ...
            app.on_do_load_all(move || {
                ...
                let msg = format!(
                    "Loaded {} transactions from database for current client.",
                    txn_count
                );
        ...
        // ── Audit Trail ───────────────────────────────────────────────────────
        {
            ...
            app.on_do_audit_trail(move |action_idx: i32, type_idx: i32| {
                ...
                let filtered: Vec<SharedString> = all_events
                    .iter()
                    .filter(|e| {
                ...
                log::info!(
                    "[AuditTrail] action_idx={} type_idx={} total_events={}",
                    action_idx,
                    type_idx,
                    all_events.len()
                );
        ...
        // ── Export Reconciliation CSV ─────────────────────────────────────────
        {
            ...
            app.on_do_export_recon_csv(move || {
                ...
                if csv.is_empty() {
                    h.set_status_bank(SharedString::from(
                        "Run a reconciliation first before exporting.",
                    ));
                    return;
                }
                let path = match rfd::FileDialog::new()
        ...
        // ── Settings ──────────────────────────────────────────────────────────
        {
            ...
            app.on_do_settings(move || {
                ...
                if let Some(conn) = db.as_ref() {
                    ...
                    st.ai_enabled = cfg.ai_enabled;
                    log::info!(
                        "[Settings] loaded: provider='{}' enabled={}",
                        cfg.ai_provider,
                        cfg.ai_enabled
                    );
                    drop(db);
        ...
        // ── Settings Save (AI modal) ──────────────────────────────────────────
        {
            let db_ref = db_conn.clone();
            let state_ref = app_state.clone();
            app.on_do_settings_save(
                move |provider: SharedString, key: SharedString, enabled: bool| {
                    let db = db_ref.lock().unwrap();
                    if let Some(conn) = db.as_ref() {
                ...
                if let Some(conn) = db.as_ref() {
                    ...
                    match cfg.save(conn) {
                        Ok(_) => {
                            log::set_max_level(log_level_filter(&cfg.log_level));
                            log::info!(
                                "[SettingsSaveAll] saved recon_days={} recon_pct={} log_level={}",
                                cfg.recon_days,
                                cfg.recon_pct,
                                cfg.log_level
                            );
                        ...
                        Ok(json) => {
                            ...
                            match std::fs::write(&path, json) {
                                Ok(_) => {
                                    ...
                                    h.set_toast_msg(slint::SharedString::from(
                                        format!("Rules backed up to {}", path).as_str(),
                                    ));
                                    h.set_toast_kind(1);
                                }
                                Err(e) => {
                        ...
                        if let Some(conn) = db.as_ref() {
                            match db::import_rules_json(conn, cid, &json) {
                                Ok(n) => {
                                    ...
                                    h.set_toast_msg(slint::SharedString::from(
                                        format!("{} rules restored", n).as_str(),
                                    ));
                                    h.set_toast_kind(1);
                                }
                                Err(e) => {
        ...
        // ── Select Client ─────────────────────────────────────────────────────
        {
            ...
            app.on_do_select_client(move |name: SharedString| {
                ...
                if name_str.is_empty() || name_str == "-- Select Client --" {
                    return;
                }
                ...
                let import_recs: Vec<SharedString> = imports
                    .iter()
                    .map(|imp| {
                        let date = &imp.imported_at[..imp.imported_at.len().min(16)];
                        SharedString::from(
                ...
                if let Some(conn) = db.as_ref() {
                    if let Err(e) =
                        db::set_setting(conn, settings::KEY_LAST_CLIENT, &client.id.to_string())
                    {
                ...
                log::info!(
                    "[SelectClient] loaded client '{}' id={} txns={}",
                    name_str,
                    client.id,
                    txns.len()
                );
        ...
        // ── Delete Client ─────────────────────────────────────────────────────
        {
            ...
            app.on_do_delete_client(move || {
                ...
                h.set_transaction_rows(slint::ModelRc::new(slint::VecModel::from(
                    Vec::<TxnRow>::new(),
                )));
                h.set_import_records(slint::ModelRc::new(slint::VecModel::from(Vec::<
                    SharedString,
                >::new(
                ))));
        ...
        // ── Edit Client ───────────────────────────────────────────────────────
        {
            ...
            app.on_do_edit_client(move || {
                ...
                if updated.is_some() {
                    ...
                    log::info!(
                        "[EditClient] updated cid={} name='{}' ledger='{}'",
                        cid,
                        new_name,
                        new_ledger
                    );
        ...
        // ── Reload Import ─────────────────────────────────────────────────────
        {
            ...
            app.on_do_reload_import(move |idx: i32| {
                ...
                let Some(&import_id) = import_ids.get(idx as usize) else {
                    log::warn!(
                        "[ReloadImport] idx {} out of range (import_ids len={})",
                        idx,
                        import_ids.len()
                    );
                    return;
        ...
        // ── Add Row ────────────────────────────────────────────────────────────
        {
            ...
            app.on_do_add_row(move || {
                ...
                log::info!(
                    "[AddRow] modal opened — {} transactions in session",
        ...
        fn visible_to_abs(st: &ui::AppState, vis_idx: usize) -> Option<usize> {
            ...
            for (abs, t) in st.transactions.iter().enumerate() {
                ...
                if !st.date_from.is_empty() || !st.date_to.is_empty() {
                    ...
                    if !st.date_from.is_empty() {
                        if let Some(fd) = parse_date_ymd(&st.date_from) {
                            if td < fd {
                                continue;
                            }
                        }
                    }
                    if !st.date_to.is_empty() {
                        if let Some(tod) = parse_date_ymd(&st.date_to) {
                            if td > tod {
                                continue;
                            }
                        }
                ...
                if !st.bank_filter.is_empty() && t.bank_name != st.bank_filter {
                    continue;
                }
                if vis == vis_idx {
                    return Some(abs);
                ...
                if let Some((cid, event_str, txn_id)) = audit {
                    ...
                    if let Some(conn) = db.as_ref() {
                        ...
                        if let Err(e) = db::delete_transaction(conn, cid, &txn_id) {
                            ...
                            h.set_toast_msg(SharedString::from(
                                format!("Delete failed — row will reappear on reload: {}", e)
                                    .as_str(),
                            ));
                            h.set_toast_kind(2);
                        }
...

        {
            ...
            app.on_do_pdf_pwd_confirm(move |pwd: SharedString| {
                ...
                let (pending_path, pending_name, is_batch) = {
                    let st = state_ref.lock().unwrap();
                    (
                        st.pending_pdf_path.clone(),
                        st.pending_pdf_name.clone(),
                        st.batch_progress.is_some(),
                    )
                };
                ...
                let parse_result = if stage1.is_some() {
                    ...
                    let full_text =
                        parser::text_extractor::extract_full_text_with_password(&path, &pwd_bytes);
                    if full_text.trim().is_empty() {
                        ...
                        if real_count > 0 {
                            ...
                            if !preprocessed.trim().is_empty() {
                                let ml =
                                    parser::ocr_parser::parse_ocr_text(&preprocessed, &file_name);
                                let ml_count = ml
                                    .transactions
                                    .iter()
                                    .filter(|t| !t.is_opening_balance)
                    ...
                    // mode (other files still complete).
                    {
                        ...
                        if let Some(bp) = st.batch_progress.as_mut() {
                            record_batch_failure(
                                bp,
                                &pending_name,
                                "Password required \u{2014} cancelled",
                            );
                        }
                    }
                    continue_batch(&h, &state_ref, &db_ref);
        ...
        // ── Dashboard filter callback ──────────────────────────────────────────
        {
            ...
            app.on_do_dash_filter(move |from, to, bank, vendor, head| {
                ...
                let filtered_txns: Vec<parser::Transaction> = if from.is_empty()
                    && to.is_empty()
                    && bank.is_empty()
                    && vendor.is_empty()
                    && head.is_empty()
