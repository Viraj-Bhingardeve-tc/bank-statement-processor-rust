...
static RE_OPENING_BAL: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)opening\s*(balance|bal)").unwrap());
static RE_CLOSING_BAL: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)closing\s*(balance|bal)").unwrap());
...
impl GridCell {
    pub fn from_data_type(dt: &Data) -> Self {
        match dt {
            ...
            Data::DateTimeIso(s) | Data::DurationIso(s) => GridCell::Text(s.clone()),
        }
    }
    ...
    pub fn as_date(&self) -> super::date_parser::ParsedDate {
        match self {
            GridCell::Empty => super::date_parser::ParsedDate {
                display: String::new(),
                ts: 0,
                valid: false,
            },
            GridCell::Serial(f) => normalize_excel_date(*f)
            ...
            GridCell::Number(f) => {
                ...
                if *f > EXCEL_DATE_MIN && *f < EXCEL_DATE_MAX {
                    if let Some(pd) = normalize_excel_date(*f) {
                        return pd;
                    }
                }
...
pub fn detect_cols_from_content(
    ...
    let stats: Vec<ColStats> = (0..max_col)
        .map(|c| {
            ...
            let date_cnt = vals
                .iter()
                .filter(|v| normalize_transaction_date(v).valid)
                .count();
            let num_cnt = vals
                .iter()
                .filter(|v| parse_amount_str(v).is_some())
                .count();
            let txt_cnt = vals
                .iter()
                .filter(|v| {
    ...
    if existing.debit < 0 && existing.credit < 0 {
        let mut num_cols: Vec<usize> = stats
            .iter()
            .enumerate()
        ...
        if num_cols.len() >= 2 {
            ...
            log::debug!(
                "[BSP Content] debit ← col {}, credit ← col {}",
                num_cols[0],
                num_cols[1]
            );
            ...
            taken.insert(num_cols[0] as i32);
            log::debug!(
                "[BSP Content] debit ← col {} (single amount col)",
                num_cols[0]
            );
        }
    }
    ...
    if op_bal.is_none() {
        for t in txns.iter() {
            ...
            if let (Some(bal), true) = (t.balance, has_amt) {
                ...
                log::debug!(
                    "[BSP Opening Balance] Derived: {:.2}  (first balance={:.2}  net={:+.2})",
                    op_bal.unwrap(),
                    bal,
                    net
                );
    ...
    for t in txns.iter_mut() {
        ...
        if let Some(rb) = run_bal {
            ...
            if let Some(stated) = t.balance {
                ...
                if diff > 1.0 {
                    log::warn!(
                        "[BSP Opening Balance] MISMATCH exp={:.2} got={:.2} Δ={:.2}  \"{}\"",
                        expected,
                        stated,
                        diff,
                        safe_prefix(&t.narration, 35)
                    );
                }
...
pub fn correct_debit_credit_by_balance(txns: &mut [Transaction]) {
    ...
    for t in txns.iter_mut() {
        ...
        match (t.debit, t.credit) {
            (Some(dr), None) if (diff - dr).abs() < tol(dr) => {
                ...
                t.debit = None;
                log::debug!(
                    "[BSP Correct] debit→credit  Δbal={:+.2}  amt={:.2}  \"{}\"",
                    diff,
                    dr,
            ...
            (None, Some(cr)) if (diff + cr).abs() < tol(cr) => {
                ...
                t.credit = None;
                log::debug!(
                    "[BSP Correct] credit→debit  Δbal={:+.2}  amt={:.2}  \"{}\"",
                    diff,
                    cr,
                    safe_prefix(&t.narration, 40)
                );
...
pub fn validate_balances(
    ...
    if mismatch_count > 0 {
        log::warn!(
            "[BSP Bal] \"{}\": {} mismatch(es) / {} ok / {} without-balance",
            source_name,
            mismatch_count,
            ok_count,
            skip_count
        ...
        log::debug!(
            "[BSP Bal] \"{}\": all {} balances reconcile ✓ ({} without-balance skipped)",
            source_name,
            ok_count,
            skip_count
        );
...
pub fn extract_sheet_from_grid(grid: &[Vec<GridCell>], sheet_name: &str) -> Option<ParseResult> {
    ...
    for (i, row) in grid
        .iter()
        .enumerate()
        .take(grid.len().min(MAX_HEADER_SCAN))
    {
        let row_strs: Vec<String> = row.iter().map(|c| c.as_detect_str()).collect();
    ...
    for (i, row) in grid.iter().enumerate().skip(hdr_idx + 1) {
        ...
        if let (Some(dr), None) = (final_debit, final_credit) {
            if dr < 0.0 {
                final_debit = Some(dr.abs());
            }
        }
        if let (None, Some(cr)) = (final_debit, final_credit) {
            if cr < 0.0 {
                final_credit = Some(cr.abs());
            }
        }
        ...
        if final_debit.is_some() && final_credit.is_none() && col_map.credit < 0 {
            ...
            let stripped: String = raw_dr_str
                .chars()
                .filter(|&c| c != '₹' && !c.is_whitespace() && c != ',')
                .collect();
        ...
        if final_debit.is_some() && final_credit.is_none() && RE_CR_SUFFIX.is_match(&raw_dr_str) {
            final_credit = final_debit;
            final_debit = None;
        }
        if final_credit.is_some() && final_debit.is_none() && RE_DR_SUFFIX.is_match(&raw_cr_str) {
        ...
        'outer: for row in grid[..hdr_idx].iter() {
            ...
            if RE_CLOSING_BAL.is_match(&joined) {
                ...
                if !amounts.is_empty() {
                    ...
                    log::debug!(
                        "[BSP Pre-hdr] closing balance = {}",
                        closing_balance.unwrap()
                    );
                    break 'outer;
                }
            }
...
mod tests {
    ...
    fn g_text(s: &str) -> GridCell {
        GridCell::Text(s.to_owned())
    }
    fn g_num(f: f64) -> GridCell {
        GridCell::Number(f)
    }
    ...
    fn row(cells: &[GridCell]) -> Vec<GridCell> {
        cells.to_vec()
    }
    ...
    fn hdfc_grid() -> Vec<Vec<GridCell>> {
        ...
        let mut g = vec![
            ...
            row(&[
                ...
                g_text("Value Dt"),
                g_text("Chq/Ref No."),
                g_text("Withdrawal Amt."),
                g_text("Deposit Amt."),
                g_text("Closing Balance"),
            ]),
            // Actual header row
            row(&[
                g_text("Date"),
        ...
        let txns = vec![
            row(&[
                g_date(2, 1, 2024),
                g_text("NEFT/RTG234567891/RATAN TATA/AXIS0001234"),
                g_date(2, 1, 2024),
                g_text("RTG234567891"),
                g_empty(),
                g_num(50000.0),
            ...
            row(&[
                g_date(3, 1, 2024),
                g_text("ATM WDL/ATM123456/HDFC BANK ATM"),
                g_date(3, 1, 2024),
                g_text("ATM123456"),
                g_num(10000.0),
                g_empty(),
            ...
            row(&[
                g_date(5, 1, 2024),
                g_text("SALARY CREDIT ACME PVT LTD JAN 2024"),
                g_date(5, 1, 2024),
                g_text("SAL00001"),
                g_empty(),
                g_num(80000.0),
            ...
            row(&[
                g_date(7, 1, 2024),
                g_text("SWIGGY TECHNOLOGIES PVT LTD"),
                g_date(7, 1, 2024),
                g_text("9823456789"),
                g_num(850.0),
                g_empty(),
            ...
            row(&[
                g_date(8, 1, 2024),
                g_text("BPCL FUEL STATION MUMBAI"),
                g_date(8, 1, 2024),
                g_text("POS98765"),
                g_num(3500.0),
                g_empty(),
            ...
            row(&[
                g_date(10, 1, 2024),
                g_text("MSEDCL ELECTRICITY BILL JAN 2024"),
                g_date(10, 1, 2024),
                g_text("MSEDCL2024"),
                g_num(2800.0),
                g_empty(),
            ...
            row(&[
                ...
                g_empty(),
                g_num(197850.0),
            ]), // exact duplicate (same balance → caught by dedup key)
            row(&[
                g_date(12, 1, 2024),
                g_text("UPI/CR/234567890123/MAHESH KUMAR/mahesh@okaxis"),
                g_date(12, 1, 2024),
                g_text("UPI234567"),
                g_empty(),
                g_num(15000.0),
            ...
            row(&[
                g_date(15, 1, 2024),
                g_text("GST PMT CGST SGST CHALLAN 09-24"),
                g_date(15, 1, 2024),
                g_text("NSDL8765432"),
                g_num(18000.0),
                g_empty(),
            ...
            row(&[
                g_date(18, 1, 2024),
                g_text("AMAZON PAY INDIA PVT LTD"),
                g_date(18, 1, 2024),
                g_text("AMZ98765432"),
                g_num(4500.0),
                g_empty(),
            ...
            row(&[
                g_date(20, 1, 2024),
                g_text("LIC PREMIUM POLICY NO 123456789"),
                g_date(20, 1, 2024),
                g_text("LIC78901234"),
                g_num(12000.0),
                g_empty(),
            ...
            row(&[
                g_date(22, 1, 2024),
                g_text("INTEREST CREDITED FOR JAN 2024"),
                g_date(22, 1, 2024),
                g_text("INT00001"),
                g_empty(),
                g_num(850.0),
            ...
            row(&[
                g_date(25, 1, 2024),
                g_text("RENT PAYMENT TO OWNER VIA NEFT"),
                g_date(25, 1, 2024),
                g_text("NEFT2500001"),
                g_num(35000.0),
                g_empty(),
            ...
            row(&[
                g_date(28, 1, 2024),
                g_text("INCOME TAX ADVANCE TAX Q3 CHALLAN 280"),
                g_date(28, 1, 2024),
                g_text("ITAX00001"),
                g_num(25000.0),
                g_empty(),
            ...
            row(&[
                g_date(30, 1, 2024),
                g_text("ZERODHA BROKING SIP AXIS MF"),
                g_date(30, 1, 2024),
                g_text("ZER001234"),
                g_num(5000.0),
                g_empty(),
    ...
    fn sbi_grid() -> Vec<Vec<GridCell>> {
        ...
        let mut g = vec![
            ...
            row(&[g_text(
                "Acc No: 30120456789 | A S HAVALDAR | SAVINGS | MUMBAI MAIN",
            )]),
            // Opening B/F row
            row(&[
                g_empty(),
        ...
        let txns = vec![
            row(&[
                g_date(1, 3, 2024),
                g_date(1, 3, 2024),
                g_text("BY TRANSFER-CR-NEFT-UTR876543210-RAJESH SHAH"),
                g_text("UTR876543210"),
                g_empty(),
                g_num(25000.0),
                g_num(87000.0),
            ...
            row(&[
                g_date(4, 3, 2024),
                g_date(4, 3, 2024),
                g_text("TO TRANSFER-DR-UPI/P2P/OLA CABS/OLA"),
                g_text("UPI78901234"),
                g_num(350.0),
                g_empty(),
                g_num(86650.0),
            ...
            row(&[
                g_date(6, 3, 2024),
                g_date(6, 3, 2024),
                g_text("TO TRANSFER-DR-UBER TECHNOLOGIES INC"),
                g_text("UBER345678"),
                g_num(420.0),
                g_empty(),
                g_num(86230.0),
            ...
            row(&[
                g_date(8, 3, 2024),
                g_date(8, 3, 2024),
                g_text("SAL/MARCH/2024/XYZ COMPANY PVT LTD"),
                g_text("SAL03240001"),
                g_empty(),
                g_num(55000.0),
                g_num(141230.0),
            ...
            row(&[
                g_date(10, 3, 2024),
                g_date(10, 3, 2024),
                g_text("TO BPCL PETROL PUMP DADAR MUMBAI"),
                g_text("BPCL2024003"),
                g_num(4200.0),
                g_empty(),
                g_num(137030.0),
            ...
            row(&[
                g_date(12, 3, 2024),
                g_date(12, 3, 2024),
                g_text("MSEDCL ELECTRICITY BILL MARCH"),
                g_text("ELB2024003"),
                g_num(3100.0),
                g_empty(),
                g_num(133930.0),
            ...
            row(&[
                g_date(14, 3, 2024),
                g_date(14, 3, 2024),
                g_text("ATM CASH WDL SBI ATM BANDRA"),
                g_text("ATM234567"),
                g_num(20000.0),
                g_empty(),
                g_num(113930.0),
            ...
            row(&[
                g_date(15, 3, 2024),
                g_date(15, 3, 2024),
                g_text("GST CHALLAN IGST CGST SGST PMT REF 09-2024"),
                g_text("GST2024003"),
                g_num(27000.0),
                g_empty(),
                g_num(86930.0),
            ...
            row(&[
                g_date(18, 3, 2024),
                g_date(18, 3, 2024),
                g_text("ZERODHA BROKING LTD SIP PARAG PARIKH"),
                g_text("ZER2024031"),
                g_num(10000.0),
                g_empty(),
                g_num(76930.0),
            ...
            row(&[
                g_date(20, 3, 2024),
                g_date(20, 3, 2024),
                g_text("MEDPLUS HEALTH SERVICES PVTLTD"),
                g_text("MED2024002"),
                g_num(1850.0),
                g_empty(),
                g_num(75080.0),
            ...
            row(&[
                g_date(22, 3, 2024),
                g_date(22, 3, 2024),
                g_text("INTEREST CREDITED SB A/C MARCH"),
                g_text("INT2024003"),
                g_empty(),
                g_num(320.0),
                g_num(75400.0),
            ...
            row(&[
                g_date(23, 3, 2024),
                g_date(23, 3, 2024),
                g_text("MSEDCL ELECTRICITY BILL MARCH REPOST"),
                g_text("ELB2024003B"),
                g_num(3100.0),
                g_empty(),
                g_num(72300.0),
            ...
            row(&[
                g_date(25, 3, 2024),
                g_date(25, 3, 2024),
                g_text("INCOME TAX ADVANCE TAX CHALLAN 280 Q4"),
                g_text("ITAX2024Q4"),
                g_num(15000.0),
                g_empty(),
                g_num(57300.0),
            ...
            row(&[
                g_date(28, 3, 2024),
                g_date(28, 3, 2024),
                g_text("LIC PREMIUM JEEVAN ANAND POLICY NO 876543"),
                g_text("LIC876543"),
                g_num(8500.0),
                g_empty(),
                g_num(48800.0),
            ...
            row(&[
                g_date(31, 3, 2024),
                g_date(31, 3, 2024),
                g_text("DIVIDEND CREDIT INFOSYS LTD Q4"),
                g_text("DIV00456"),
                g_empty(),
                g_num(2100.0),
                g_num(50900.0),
    ...
    fn continuation_grid() -> Vec<Vec<GridCell>> {
        ...
        vec![
            ...
            row(&[
                g_date(10, 1, 2024),
                g_text("NEFT/CR/UTR123456/INFOSYS LTD"),
                g_empty(),
                g_num(50000.0),
                g_num(150000.0),
            ]),
            row(&[
                g_empty(),
                g_text("Bangalore / Mysore Division Q3"),
                g_empty(),
                g_empty(),
                g_empty(),
            ]),
    ...
    fn hdfc_col_map() {
        ...
        assert_eq!(
            result.header_row_idx, 10,
            "header at row 10 (first full keyword match)"
        );
    }
    ...
    fn hdfc_txn_count() {
        ...
        assert_eq!(
            real_txns.len(),
            14,
            "14 real transactions (1 exact dup removed)"
        );
    }
    ...
    fn excel_path_detects_bank_and_account() {
        ...
        assert!(
            result.account_no.ends_with("6789"),
            "account_no = {:?}",
            result.account_no
        );
        assert!(
            result.account_no.contains('X'),
    ...
    fn sbi_opening_balance() {
        ...
        assert!(
            (ob - 62000.0).abs() < 0.5,
            "B/F row captured as opening balance, got {}",
            ob
        );
    }
    ...
    fn kotak_negative_is_debit() {
        ...
        assert!(
            (uber.debit.unwrap() - 650.0).abs() < 0.01,
            "abs value = 650"
        );
    }
    ...
    fn kotak_positive_is_credit() {
        ...
        assert!(
            salary.credit.is_some(),
            "positive DEBIT/CREDIT amount → credit"
        );
    ...
    fn noise_rows_not_in_txns() {
        ...
        for t in &result.transactions {
            assert!(
                ...
                "Grand Total leaked into transactions: {:?}",
                t.narration
            );
        }
    }
    ...
    fn compute_prev_balances_derives_ob() {
        ...
        assert!(
            (ob.unwrap() - 135000.0).abs() < 0.5,
            "derived OB = 135000, got {:?}",
            ob
        );
        assert!(
            (txns[0].prev_balance.unwrap() - 135000.0).abs() < 0.5,
            "first txn prev_balance = OB"
        );
        assert!(
            (txns[1].prev_balance.unwrap() - 125000.0).abs() < 0.5,
    ...
    fn correct_debit_credit_swapped_debit() {
        ...
        let mut txns = vec![
            Transaction {
                balance: Some(100000.0),
                debit: Some(5000.0),
                ..Transaction::new("t1")
            },
            Transaction {
                balance: Some(105000.0),
                debit: Some(5000.0),
                ..Transaction::new("t2")
            },
        ];
    ...
    fn correct_debit_credit_no_swap_when_correct() {
        ...
        correct_debit_credit_by_balance(&mut txns);
        assert!(
            txns[1].debit.is_some(),
            "debit NOT swapped (balance moved correctly)"
        );
    ...
    fn deduplicate_keeps_different_txns() {
        let t1 = Transaction {
            date: "01/01/2024".into(),
            narration: "TXN A".into(),
            debit: Some(1000.0),
            ..Transaction::new("t1")
        };
        let t2 = Transaction {
            date: "01/01/2024".into(),
            narration: "TXN B".into(),
            debit: Some(1000.0),
            ..Transaction::new("t2")
        };
        let result = deduplicate_txns(vec![t1, t2]);
    ...
    fn prepend_ob_derives_from_first_txn() {
        ...
        assert!(
            (txns[0].balance.unwrap() - 135000.0).abs() < 0.5,
    ...
    fn prepend_ob_uses_fallback_when_no_balance_col() {
        ...
        assert!(txns[0].is_opening_balance);
        assert!(
            (txns[0].balance.unwrap() - 5000.0).abs() < 0.5,
    ...
    fn prepend_ob_id_is_identical_across_repeated_imports_of_the_same_statement() {
        let mut txns_first_import = vec![sample_first_txn(
            "01/04/2024",
            "NEFT SALARY CREDIT",
            Some(10000.0),
        )];
        let mut txns_second_import = vec![sample_first_txn(
            "01/04/2024",
            "NEFT SALARY CREDIT",
            Some(10000.0),
        )];
    ...
    fn prepend_ob_does_not_change_existing_transaction_ids() {
        let mut txns = vec![
            Transaction {
                ...
                debit: Some(10000.0),
                balance: Some(125000.0),
                ..Transaction::new("t_0_2")
            },
            Transaction {
                id: "t_1_2".to_string(),
                credit: Some(2000.0),
                balance: Some(127000.0),
                ..Transaction::new("t_1_2")
            },
        ];
        prepend_opening_balance_row(&mut txns, None, "TestBank", "ACC001");
        assert_eq!(
            txns.len(),
            3,
            "OB row prepended in front of the 2 real rows"
        );
        assert_eq!(
            txns[1].id, "t_0_2",
            "first real transaction's id must be untouched"
        );
        assert_eq!(
            txns[2].id, "t_1_2",
            "second real transaction's id must be untouched"
        );
    }
    ...
    fn detect_cols_from_content_infers_balance() {
        ...
        assert_eq!(
            updated.balance, 4,
            "balance inferred as col 4 (highest numeric%)"
        );
    }
    ...
    fn detect_cols_from_content_early_return_when_few_samples() {
        ...
        assert_eq!(
            updated.balance, -1,
            "balance not inferred (< 3 sample rows)"
        );
    }
    ...
    fn cr_suffix_in_debit_col_becomes_credit() {
        ...
        let grid = vec![
            ...
            row(&[
                g_date(1, 1, 2024),
                g_text("Payment received"),
                g_text("1,500.00 Cr"),
                g_num(101500.0),
            ]),
            row(&[
                g_date(2, 1, 2024),
                g_text("Purchase made"),
                g_text("500.00"),
                g_num(101000.0),
            ]),
        ];
    ...
    fn hdfc_reconciliation_passes() {
        ...
        let total_dr: f64 = result
            .transactions
            .iter()
            .filter(|t| !t.is_opening_balance)
            .map(|t| t.debit.unwrap_or(0.0))
            .sum();
        let total_cr: f64 = result
            .transactions
            .iter()
            .filter(|t| !t.is_opening_balance)
        ...
        assert!(
            (calc_cl - stated_cl).abs() < 5000.0,
            "reconciliation within tolerance: calc={} stated={}",
            calc_cl,
            stated_cl
        );
    }
