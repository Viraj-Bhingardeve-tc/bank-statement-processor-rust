...
pub fn validate_transaction(t: &Transaction) -> (bool, String) {
    ...
    match amt {
        ...
        Some(a) => {
            if !a.is_finite() || a.abs() > 2e9 {
                reasons.push("Amount exceeds \u{20B9}200 crore");
            }
            if a < 0.0 {
                reasons.push("Negative amount — check column detection");
            }
        }
    }
    if t.narration.trim().len() < 2 {
        reasons.push("Empty or very short narration");
    }
    if let Some(bal) = t.balance {
        if bal < -1_000_000.0 {
            reasons.push("Suspiciously negative balance");
        }
    }
    if t.dup_flag {
        reasons.push("Possible duplicate transaction");
    }
...
fn fmt_short(v: f64) -> String {
    if v >= 1_000_000.0 {
        return format!("₹{:.1}L", v / 100_000.0);
    }
    if v >= 1_000.0 {
        return format!("₹{:.0}K", v / 1_000.0);
    }
    fmt_inr(v)
}
...
pub fn filter_txns<'t>(txns: &'t [Transaction], f: &DashFilter) -> Vec<&'t Transaction> {
    txns.iter()
        .filter(|t| {
            ...
            if !f.bank.is_empty() && t.bank_name != f.bank {
                return false;
            }
            if !f.vendor.is_empty() && t.vendor != f.vendor {
                return false;
            }
            if !f.head.is_empty() && t.account_head != f.head {
                return false;
            }
...
pub fn compute(txns: &[Transaction], opening_bal: Option<f64>) -> AnalyticsResult {
    ...
    let expenses: Vec<ExpHead> = exp_top10
        ...
        .enumerate()
        .map(|(i, (label, amt))| {
            let pct = if exp_total > 0.0 {
                (amt / exp_total * 100.0).round() as i32
            } else {
                0
            };
            ExpHead {
    ...
    let bal_txns: Vec<f64> = real.iter().filter_map(|t| t.balance).collect();
    let step = if bal_txns.len() > 150 {
        bal_txns.len() / 150
    } else {
        1
    };
    ...
    let mut vmap: std::collections::HashMap<&str, (f64, f64)> = std::collections::HashMap::new();
    for t in &real {
        if !t.vendor.is_empty() {
    ...
    let mut vvec: Vec<(&str, f64, f64)> = vmap.iter().map(|(n, (d, c))| (*n, *d, *c)).collect();
    vvec.sort_by(|a, b| {
        (b.1 + b.2)
            .partial_cmp(&(a.1 + a.2))
            .unwrap_or(std::cmp::Ordering::Equal)
    });
    let vendors: Vec<VendorAgg> = vvec
        .iter()
        .take(10)
        .map(|(n, d, c)| VendorAgg {
            name: n.to_string(),
    ...
    // ── Insights ──────────────────────────────────────────────────────────────
    let dr_txns: Vec<&&Transaction> = real
        .iter()
        .filter(|t| t.debit.unwrap_or(0.0) > 0.0)
        .collect();
    let cr_txns: Vec<&&Transaction> = real
        .iter()
        .filter(|t| t.credit.unwrap_or(0.0) > 0.0)
        .collect();
    ...
    let avg_dr = if dr_txns.is_empty() {
        0.0
    } else {
        total_debit / dr_txns.len() as f64
    };
    let avg_cr = if cr_txns.is_empty() {
        0.0
    } else {
        total_credit / cr_txns.len() as f64
    };
    ...
    for t in &real {
        if !t.vendor.is_empty() {
            *freq.entry(&t.vendor).or_default() += 1;
        }
    }
    let freq_vendor = freq
        .iter()
        .max_by_key(|(_, c)| *c)
        .map(|(n, c)| format!("{}  ({}×)", n, c))
    ...
    let insights = Insights {
        max_dr_amt: max_dr
            .map(|t| fmt_inr(t.debit.unwrap_or(0.0)))
            .unwrap_or("—".to_string()),
        max_dr_narr: max_dr
            .map(|t| t.narration.chars().take(45).collect())
            .unwrap_or_default(),
        max_cr_amt: max_cr
            .map(|t| fmt_inr(t.credit.unwrap_or(0.0)))
            .unwrap_or("—".to_string()),
        max_cr_narr: max_cr
            .map(|t| t.narration.chars().take(45).collect())
            .unwrap_or_default(),
        avg_dr: if avg_dr > 0.0 {
            fmt_inr(avg_dr)
        } else {
            "—".to_string()
        },
        avg_cr: if avg_cr > 0.0 {
            fmt_inr(avg_cr)
        } else {
            "—".to_string()
        },
        dr_count: if dr_txns.is_empty() {
            "—".to_string()
        } else {
            format!("{} debits", dr_txns.len())
        },
        cr_count: if cr_txns.is_empty() {
            "—".to_string()
        } else {
            format!("{} credits", cr_txns.len())
        },
        freq_vendor,
    };
...
pub fn unique_banks(txns: &[Transaction]) -> Vec<String> {
    ...
    for t in txns {
        if !t.is_opening_balance && !t.bank_name.is_empty() {
            set.insert(&t.bank_name);
        }
    }
...
pub fn unique_vendors(txns: &[Transaction]) -> Vec<String> {
    let mut set = std::collections::BTreeSet::new();
    for t in txns {
        if !t.is_opening_balance && !t.vendor.is_empty() {
            set.insert(&t.vendor);
        }
    }
    set.into_iter().cloned().collect()
...
pub fn unique_heads(txns: &[Transaction]) -> Vec<String> {
    let mut set = std::collections::BTreeSet::new();
    for t in txns {
        if !t.is_opening_balance && !t.account_head.is_empty() {
            set.insert(&t.account_head);
        }
    }
    set.into_iter().cloned().collect()
}
